...
- `自动对齐（OCR）`：开启后定期读取两路计时器并计算 offset。
- `对齐一次后暂停自动截图和检查`：确认对齐后停止后续自动 OCR 检查和定时截图，直播继续运行；手动同步截图仍可使用。
...
程序运行后会按间隔从视频源和音频源各抓一帧。计时器位置查找顺序是：
...
默认只重编码输出音频为 AAC，视频仍然 `copy`，不影响 4K 画质。这样可以避免 fMP4/Emby 中出现 `track 1: codec frame size is not set` 这类音频参数警告。需要完全复制音频时，可以设置 `OUTPUT_AUDIO_CODEC=copy` 后重建容器。
...
如果 OCR 占用 CPU 高，可以适当调大“检查间隔（秒）”和“截图间隔（秒）”。
