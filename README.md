...
| `对齐一次后暂停自动截图和检查` | 确认对齐后停止后续自动截图和 OCR 检查，直播继续运行；手动截图仍可使用。 |
| `非比赛时间直播也自动对齐` | 默认关闭。关闭时，不在赛程比赛窗口内会暂停自动截图和 OCR。 |
...
| `视频计时器区域` / `音频计时器区域` | `x,y,w,h`，数值为 0 到 1 的相对坐标。 |
| `视频计时器预设区域` / `音频计时器预设区域` | 每行一个 `x,y,w,h`，用于多个常见计时器位置。 |
...
默认使用 [OCR.space](https://ocr.space) 在线识别计时器，需要注册免费账号获取 API Key。

注册地址：https://ocr.space/ocrapi （免费用户每月 25,000 次请求）

### 配置方式

1. 在 OCR.space 注册，获取 API Key。
2. 在 WebUI "OCR 识别设置" 里选择服务商并填入 API Key。
3. 保存后自动对齐会使用 OCR.space 识别计时器。

如果不配置 API Key，自动对齐会跳过 OCR 检查（不影响直播输出）。

### 工作方式

未锁定计时器位置时，OCR.space 模式使用智能扫描：

1. 发送画面上半部分给 OCR.space → 找到计时器文本和位置。
2. 根据文本在左半还是右半，下次只发送对应的四分之一区域。
3. 逐次缩小范围，快速定位计时器位置。

锁定 ROI 后，只发送 ROI 区域给 OCR.space，节省请求量。
...
| `AUTO_ALIGN_ENABLED` | `1` | 是否启用自动 OCR 对齐。 |
...
| `AUTO_ALIGN_STOP_AFTER_ALIGNED` | `0` | 对齐一次后是否暂停自动截图和检查。 |
...
如果 OCR 占用 CPU 高，可以调大“截图/检查间隔（秒）”，或开启“对齐一次后暂停自动截图和检查”。
